...
public class Solution {
    public int[] solve(int[] A) {
        Arrays.sort(A);
        int i,min=0,max=0,l=A.length,mod=1000000007;
        for(i=0;i<l/2;i++)
        {
           min=(min%mod+(mod+A[i])%mod)%mod; //A[i]+mod is very important since numbers can be negative also
        }
        for(i=l/2;i<l;i++)
        {
            max=(max%mod+(mod+A[i])%mod)%mod;   //A[i]+mod is very important as if the number is negative so addng mod so that we get positive value only (-2)%5=3 but computer gives -2 only
        }
        min=(max-min+mod)%mod;
        max=0;
        for(i=0;i<l;i+=2)
        {
            max=(max%mod+(A[i+1]-A[i]+mod)%mod)%mod;
        }
        int arr[]=new int[2];
        arr[0]=min;
        arr[1]=max;
