...
public class Solution {
    ...
    public int solve(int[] A, int[] B) {
        int l=A.length,i,max_d=0,val=0;
        for(i=0;i<l;i++)
            max_d=max_d%1000000007+max(0,A[(i+1)%l]-B[i])%1000000007;
        int bullets=max_d%1000000007-max(0,A[0]-B[l-1])%1000000007+A[0]%1000000007;
        for(i=1;i<l;i++)
        {
            val=max_d%1000000007-max(0,A[i]-B[i-1])%1000000007+A[i]%1000000007;
            if(val<bullets)
                bullets=val;
        }
        return bullets%1000000007;
    }
}
